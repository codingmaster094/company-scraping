...
exports.extractVerified = extractVerified;
/**
...
const NAV_NOISE = /^(home|about(\s+us)?|our\s+team|team|blog|news|contact|get\s+a\s+quote|careers?|jobs?|privacy|terms|login|shop|cart|search|portfolio|work|case\s+studies|reviews?|testimonials?|locations?|faq|resources?|white\s*label)$/i;
const SERVICE_RULES = [
...
const HIGH_BEFORE = String.raw `(?:we\s+(?:provide|offer|specialize\s+in|are\s+experts?\s+in|build\s+(?:on|with)|work\s+with|develop(?:\s+on|\s+in)?|create)|` +
...
const MEDIUM_SECTION = /\b(platform(?:s)?\s+(?:we\s+work\s+with|expertise)|technologies\s+we\s+work\s+with|our\s+platforms?|tech(?:nology)?\s+stack\s+we|cms\s+we|ecommerce\s+platforms?|we\s+build\s+(?:on|across)|short\s+list\s+of\s+our\s+platform)\b/i;
...
function extractScopedLabels(html) {
...
function mapServiceLabel(label) {
    const t = label.trim();
    if (SERVICE_NEVER_INFER.some((rx) => rx.test(t)))
        return null;
...
function evidenceForToken(copy, token) {
    ...
    while ((m = re.exec(copy)) !== null) {
        ...
        const navOffer = new RegExp(`${token.source}\\s+(?:web\\s+)?(?:site\\s+)?(?:design|development|stores?|websites?|packages?)`, "i");
...
function extractCompanySupportedTech(htmls, labels) {
    const copy = htmls.map(visibleCopy).concat(labels).join(" \n ");
    const found = [];
    const claimed = new Set();
    for (const tech of OFFERED_TECH) {
        if (NEVER_TECH.has(tech.name))
            continue;
        if (claimed.has(tech.name))
            continue;
...
function extractVerified(options) {
    ...
    for (const html of htmls)
        labels.push(...extractScopedLabels(html));
    const services = unique(labels.map(mapServiceLabel).filter((s) => Boolean(s)));
