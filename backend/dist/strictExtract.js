...
const NEVER_TECH = new Set([
  "Cloudflare", "Google Analytics", "Google Tag Manager", "jQuery",
  "HTML", "HTML5", "CSS", "CSS3", "Sass", "Babel", "Lodash", "Sentry",
  "Webpack", "Vite", "CDN",
]);

const HIGH_AFTER =
  "(?:\\s+(?:cms|framework|platform|plus))?\\s+(?:development|developers?|design(?:ing)?|services?|experts?|specialists?|agency|websites?|stores?|apps?|applications?)";

const HIGH_BEFORE =
  "(?:we\\s+(?:provide|offer|specialize\\s+in|are\\s+experts?\\s+in|build\\s+(?:on|with)|work\\s+with|develop(?:\\s+on|\\s+in)?|create)|our\\s+expertise\\s+(?:includes?|in)|expertise\\s+(?:includes?|in)|speciali[sz]e(?:s|d)?\\s+in|technologies\\s+we\\s+work\\s+with|platforms?\\s+we\\s+work\\s+with|platform\\s+expertise)";
...
function visibleCopy(html) {
  ...
  return ($("body").text() || String(html || "").replace(/<[^>]+>/g, " "))
    .replace(/\s+/g, " ")
    .trim();
}
...
function extractVerified(options) {
  ...
  const services = unique(labels.map(mapServiceLabel).filter(Boolean));
