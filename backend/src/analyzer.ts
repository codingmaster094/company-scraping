 ...
 */
import { extractVerified } from "./strictExtract";

export interface AnalyzerOptions {
  /** Additional text from service-related subpages (combined after tag stripping). */
  servicePageText?: string;
  /** Raw HTML of the main page (used for tech marker detection). */
  mainHtml: string;
  /** Raw HTML of service subpages (used for tech marker detection too). */
  servicePageHtmls?: string[];
  /** Concatenated HTTP response headers (for server/CDN detection). */
  headerText?: string;
}
...
interface ServiceDef {
  name: string;
  /** Each keyword is a phrase matched with word boundaries. Weight 2 = strong, 1 = weak. */
...
type SignalType = "asset" | "html" | "meta" | "text" | "header";
...
function collectAssets(...htmls: string[]): string {
  const assets: string[] = [];
  const patterns: RegExp =
    /<(?:script|link|img|source|iframe|video|audio)[^>]+(?:src|href|data-src|srcset)\s*=\s*["']([^"']+)["']/gi;
...
function detectServicesFromText(text: string): string[] {
  ...
  const scored: Array<{ name: string; score: number; matchedKeywords: string[] }> = [];
...
export function analyzeContent(options: AnalyzerOptions): AnalysisResult {
...
function detectServices(options: AnalyzerOptions): string[] {
  return analyzeContent(options).services;
}

function detectTechnologies(options: AnalyzerOptions): string[] {
  return analyzeContent(options).technologies;
}

// Re-export for backward compatibility with scraper.ts
export { detectServices, detectTechnologies };